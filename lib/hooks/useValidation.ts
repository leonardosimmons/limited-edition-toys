...
function useValidation() {
  ...
  function password(field: string): string {
    ...
    else if (field?.length && field.length < 6)
  ...
  function state(state: string): string {
    let fail: string = '';
    if (!states.abbrv.includes(state.toUpperCase())) {
      if (!states.full.includes(state.toLowerCase())) {
  ...
  function username(field: string): string {
    ...
    else if (field?.length && field.length < 5)
